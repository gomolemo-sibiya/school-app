...
const formSchema = z.object({
  ...
  targetRoles: z.array(z.enum(['student', 'lecturer', 'admin'])).min(1, { message: 'Select at least one target role' })
});

interface NotificationFormProps {
  notification?: Notification;
  onSubmit: (data: Omit<Notification, 'id' | 'createdAt' | 'createdBy' | 'status'>) => void;
  onCancel: () => void;
}

const NotificationForm: React.FC<NotificationFormProps> = ({ notification, onSubmit, onCancel }) => {
  ...
  const handleSubmit = (values: z.infer<typeof formSchema>) => {
    // Ensure all required fields are non-optional before passing to onSubmit
                            ...
                            <Checkbox
                              ...
                              onCheckedChange={(checked) => {
                                const updatedRoles = checked
                                  ...
                                  : field.value.filter((item) => item !== role.id);
