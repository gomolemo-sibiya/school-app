...
const AppointmentsPage = () => {
  ...
  const handleCreateAppointment = (appointmentData: Omit<Appointment, 'id' | 'status' | 'createdAt'>) => {
    try {
    ...
    <div className="p-6">
      ...
      <Tabs defaultValue="all" className="w-full">
        <TabsList className="mb-4">
          ...
          <TabsTrigger value="pending" disabled={pendingAppointments.length === 0}>
            Pending
          ...
          <TabsTrigger value="accepted" disabled={acceptedAppointments.length === 0}>Accepted</TabsTrigger>
          <TabsTrigger value="declined" disabled={declinedAppointments.length === 0}>Declined</TabsTrigger>
        </TabsList>
        ...
        <DialogContent className="sm:max-w-[525px]">
          ...
          {selectedAppointment && (
            <AppointmentForm 
              ...
              onSubmit={(data) => handleUpdateAppointment(selectedAppointment.id, data)}
          ...
          {selectedAppointment && (
            <div>
              ...
              <div className="bg-muted p-3 rounded-md mb-6">
                ...
                <p className="text-sm text-muted-foreground">{format(new Date(selectedAppointment.date), 'MMMM d, yyyy')} | {selectedAppointment.startTime} - {selectedAppointment.endTime}</p>
              </div>
...
const AppointmentCard = ({ appointment, userRole, onView, onEdit, onDelete, onStatusChange }: AppointmentCardProps) => {
  ...
  return (
    <Card>
      ...
      <CardContent className="pb-2">
        <p className="text-sm text-muted-foreground mb-2 line-clamp-2">{appointment.description}</p>
        <div className="text-sm">
          <p><span className="font-medium">{isStudent ? 'Lecturer:' : 'Student:'}</span> {isStudent ? appointment.lecturerName : appointment.studentName}</p>
        </div>
      </CardContent>
